...
# ダウンロード先フォルダ（存在しない場合は作成）
download_dir = 'C:/Development/ionic/offline_packages2'
...
# package-lock.json から情報を抽出し、CSV に保存
package_data = extract_package_info(package_lock_file)
save_to_csv(package_data, output_csv)

# CSVファイルを読み込み、URLを取得してダウンロード
def download_tgz_from_csv(csv_file, download_dir):
    with open(csv_file, 'r', encoding='utf-8') as file:
        ...
        for row in reader:
            ...
            # ファイル名をパッケージ名とバージョンから作成
            tgz_filename = f"{package_name}-{version}.tgz"
            tgz_filepath = os.path.join(download_dir, tgz_filename)

            print(f"Downloading {tgz_filename} from {url}...")
            ...
            try:
                ...
                print(f"Saved to {tgz_filepath}")
            except requests.exceptions.RequestException as e:
                print(f"Error downloading {tgz_filename}: {e}")

# ダウンロードの実行
download_tgz_from_csv(output_csv, download_dir)
